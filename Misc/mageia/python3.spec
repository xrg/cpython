...
Requires:       python-rpm-macros >= 3-9
Requires:       python3-rpm-macros >= 3-9
Conflicts:      python < 2.7.17-2
Conflicts:	%{_lib}python3-devel < 3.9.0


%package -n     python%{pybasever}
Summary:        Core of Python %{version} installation
Group:          Development/Python
...
This is a meta-package that makes %{pybasever} the default python%{familyver}
version.

Note that documentation for Python is provided in the python-docs
package.

%description -n python%{pybasever}
Python %{pybasever} interpreter. This package contains the python executable,
enough to run python%{pybasever} scripts and can be installed along any other
python%{familyver} interpreter version.
...
%{_mandir}/man1/python%{familyver}.1.*


%files -n python%{pybasever}
%{_bindir}/pydoc%{pybasever}
%{_bindir}/python%{pybasever}
%if "%{ABIFLAGS_optimized}" != "%{nil}"
%{_bindir}/python%{LDVERSION_optimized}
%{_mandir}/man1/python%{LDVERSION_optimized}.1.*
%endif
%{_bindir}/2to3-%{pybasever}
%{_mandir}/man1/python%{pybasever}.1.*
%{_mandir}/man1/python%{pybasever}m.1.*
